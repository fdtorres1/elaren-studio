...
Registered in `robots.txt`. After significant page additions, submit the sitemap through the Search Console owner UI or Sitemaps API and inspect its receipt and later crawl status. Do not use the retired sitemap ping endpoint. On September 7, 2026, the domain property and existing sitemap registration were verified; the current OAuth token permits read-only inspection, so API submission requires separately consented write scope.
